    ...
    <>
      <nav className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-300 ${navBackground} py-2.5 sm:py-4`}>
        <div className="max-w-[1280px] mx-auto px-4 xl:px-8">
          <div className="flex items-center justify-between h-[50px] sm:h-[60px] w-full">
            {searchOpen ? (
              <motion.div
              ...
              >
                ...
                <button
                  ...
                  className="px-4 py-2 text-[11px] font-black text-[#EA580C] hover:text-[#FF9800] bg-slate-50 border border-slate-100 hover:border-slate-200 transition-colors uppercase tracking-widest rounded-full shrink-0"
                >
                  Cancel
                </button>
              </motion.div>
              ...
              <>
                ...
                <div className="flex items-center gap-4 xl:gap-6">
                  <div className="flex items-center gap-4">
                    ...
                    {/* Mobile Search Button (replaces Hamburger on Right) */}
                    <button
                      onClick={() => setSearchOpen(true)}
                      className="lg:hidden w-10 h-10 flex items-center justify-center text-slate-700 hover:text-[#EA580C] rounded-full hover:bg-slate-50 transition-colors"
      ...
      <AnimatePresence>
        {mobileOpen && (
          <>
            ...
            {/* Breathtaking iOS-style Side Drawer (slides from left) */}
            <motion.div
              initial={{ x: '-100%' }}
              animate={{ x: 0 }}
              exit={{ x: '-100%' }}
              transition={{ type: 'spring', damping: 30, stiffness: 300 }}
              className="fixed left-0 top-0 bottom-0 w-[85%] max-w-[380px] z-[101] bg-white/95 backdrop-blur-3xl shadow-2xl flex flex-col lg:hidden border-r border-slate-100/50 overflow-hidden"
            >
              {/* Drawer Header */}
